...
class ENode {
  ...
  toText() {
    return `<${this.tag} class="${this.classList.join(' ')}">${this.content}</${
      this.tag
    }>`
  }
}
...
function matchAbility(node) {
  ...
  let shorty = name[0] === '!'
  if (shorty) name = name.slice(1)

  const ability = getAbility(name)
  if (!ability) return node
...
function matchItalic(node) {
  ...
  if (result) line = line.replace(result[0], ITALIC_TAG.format(result[1]))
  node.content = line
  return node
}
